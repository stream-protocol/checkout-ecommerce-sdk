...
class NoInstructionsToSendError extends SdkError {
  constructor(operation, solution, options) {
    super({
      ...
      solution: solution ?? `Ensure that the provided input has an effect on the operation. ` + `This typically happens when trying to update an account with ` + `the same data it already contains.`
    });
  }
