...
const tokenModule = () => ({
  install(metaplex) {
    // Token Program.
    metaplex.programs().register(program.tokenProgram);

    metaplex.programs().getToken = function (programs) {
      return this.get(program.tokenProgram.name, programs);
    }; // Associated Token Program.


    metaplex.programs().register(program.associatedTokenProgram);

    metaplex.programs().getAssociatedToken = function (programs) {
      return this.get(program.associatedTokenProgram.name, programs);
    }; // Operations.
