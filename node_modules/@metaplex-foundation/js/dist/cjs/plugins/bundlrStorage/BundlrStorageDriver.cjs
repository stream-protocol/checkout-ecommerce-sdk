...
class BundlrStorageDriver {
  ...
  async initBundlr() {
    ...
    let bundlr;
    if (isNode && Signer.isKeypairSigner(identity)) bundlr = await this.initNodeBundlr(address, currency, identity, options);else {
      let identitySigner;
  ...
  async initNodeBundlr(address, currency, keypair, options) {
    const bPackage = _removeDoubleDefault(await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require('@bundlr-network/client')); }));
